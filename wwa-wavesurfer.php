<?php
/*
Plugin Name: WebWay Audio Wavesurfer
Plugin URI:
Description: WW Audio Wavesurfer will help record your voice, save, download and play it in front end.
Version: 1.0.0
Author: WebWay
Author URI:
*/

function wwaw_add_styles()
{
    wp_enqueue_style('wwaw_player_style', plugins_url('css/video-js.min.css', __FILE__));
}

function wwaw_add_scripts()
{
    wp_enqueue_script('wwaw_video_min', plugins_url('js/video.min.js', __FILE__), array(), '1.0.0', true);
    wp_enqueue_script('wwaw_recorder', plugins_url('js/recorder.js', __FILE__), array(), '1.0.0', true);
    wp_enqueue_script('wwaw_wavesurfer_min', plugins_url('js/wavesurfer.min.js', __FILE__), array(), '1.0.1', true);
    wp_enqueue_script('wwaw_wavesurfer_microphone_min', plugins_url('js/wavesurfer.microphone.min.js', __FILE__), array(), '1.0.0', true);
    wp_enqueue_script('wwaw_videojs_wavesurfer_min', plugins_url('js/videojs.wavesurfer.min.js', __FILE__), array(), '1.0.0', true);
    wp_enqueue_script('wwaw_videojs_record', plugins_url('js/videojs.record.js', __FILE__), array(), '1.0.0', true);
    wp_enqueue_script('wwaw_videojs_record_recorder', plugins_url('js/videojs.record.recorderjs.js', __FILE__), array(), '1.0.0', true);
    wp_enqueue_script('wwaw_jquery_ui_widget', plugins_url('js/jquery.ui.widget.js', __FILE__), array('jquery'), '1.0.0', true);
    wp_enqueue_script('wwaw_jquery_fileupload', plugins_url('js/jquery.fileupload.js', __FILE__), array('jquery'), '1.0.0', true);
    wp_enqueue_script('wwaw_html2canvas', plugins_url('js/html2canvas.js', __FILE__), array('jquery'), '1.0.0', true);
    wp_enqueue_script('wwaw_wavesurfer_init', plugins_url('js/wwa_wavesurfer_init.js', __FILE__), array('jquery'), '1.0.0', true);
}

if (!function_exists('wwaw_add_folder')) {
    function wwaw_add_folder()
    {
        $upload_dir = wp_upload_dir();
        $upload_loc = $upload_dir['basedir'] . '/wwaw_audio';
        if (!is_dir($upload_loc)) {
            wp_mkdir_p($upload_loc);
        }
    }

    register_activation_hook(__FILE__, 'wwaw_add_folder');
}
...
{
    $upload_dir = wp_upload_dir();
    $upload_path = $upload_dir['basedir'] . '/wwaw_audio/';
    $upload_url = $upload_dir['baseurl'] . '/wwaw_audio/';
    $nonce = $_POST['nonce'];

    if (!wp_verify_nonce($nonce, 'wwaw_nonce')) {
        wp_die();
    }

    if (isset($_FILES['files'])) {
        if (!validateFile($_FILES['files'])) {
            wp_die();
        }

        $checkFile = move_uploaded_file($_FILES['files']['tmp_name'][0], $upload_path . $_FILES['files']['name'][0]);
    }

    if (isset($checkFile) && $checkFile) {
        $fileUrl = $upload_url . $_FILES['files']['name'][0];
        $fileDir = $upload_path . $_FILES['files']['name'][0];
...
{
    $name = $_POST['wwaw_png']['name'];
    $data = $_POST['wwaw_png']['img'];
    ...
    if (isset($_POST['wwaw_png'])) {
        $checkFile = file_put_contents($upload_path . $name, $png);
    }

    if (isset($checkFile) && $checkFile) {

        $fileUrl = $upload_url . $name;
        $fileDir = $upload_path . $name;
...
function validateFile($file)
{
    ...
    return $file["type"][0] == $mimeType && $fileExt == $requiredExt;
}

function wwaw_denied_upload_file()
{
    unset($_FILES['files']);
...
function wwaw_create_shortcode($atts)
{
    wwaw_add_styles();
    wwaw_add_scripts();
...
add_shortcode('wwaw_audio', 'wwaw_create_shortcode');

function wwaw_add_quicktags()
{
    ...
    <script type="text/javascript">
        QTags.addButton('wwaw_audio', 'Add Audio', '[wwaw_audio]', '', 'q', 'Adding shortcode for WWAW plugin');
    </script>
    <?php
}

add_action('admin_print_footer_scripts', 'wwaw_add_quicktags', 100);

/*
 * Start wavesurfer player
 */
...
*/
     ...
     * @since 1.0
     */
    public function render_wave_color_field()
    { // 0
    ...
    public function render_cursor_color_field()
    { // 1
    ...
    <?php endif; ?>
        ...
        <div class="wrap wavesurfer">
            ...
            <!-- HEADER -->
            <!-- Header: Title	-->
            <h1><?php _e('WWA-WaveSurfer', 'wavesurfer-wp'); ?></h1>
            ...
            <h2 class="nav-tab-wrapper">
                <a href="?page=wavesurfer-wp&tab=general"
                   class="nav-tab actions-tab <?php echo $active_tab == 'general' ? 'nav-tab-active' : ''; ?>"><?php _e('General', 'wavesurfer-wp'); ?></a>
                <a href="?page=wavesurfer-wp&tab=premium"
                   class="nav-tab license-tab <?php echo $active_tab == 'premium' ? 'nav-tab-active' : ''; ?>"><?php _e('Premium', 'wavesurfer'); ?></a>
            </h2>

            <div id="tab_container">

                <?php if ($active_tab == 'general') { ?>
                <form method="post" action="<?php if (is_multisite() && is_network_admin()) {
                    echo 'edit.php?action=update_network_options';
                } else {
                    echo 'options.php';
                } ?> "> <!-- options.php is important -->
                    <?php
        ...
        <?php } else {
            do_action('wavesurfer_wp_display_premium_page');
        }; ?>
    ...
    public function render_premium_page_free()
    {
        ...
        <p><?php _e('Add time markers, to create chapters or events list, or even interactive transcripts!', 'wavesurfer-wp'); ?>
            <a href="https://www.extremraym.com/en/wavesurfer-wp-markers/"><?php _e('Demo.', 'wavesurfer-wp'); ?></a>
        </p>
        <h4><?php _e('Plug and Play', 'wavesurfer-wp'); ?></h4>
        <p><?php _e('These extra features are packed as an add-on. No need to delete and replace the original plugin.', 'wavesurfer-wp'); ?>
            <br/><?php _e('You will still be able to benefit from translations made by the community. Also, the core is still open source, to allow contribution.', 'wavesurfer-wp'); ?>
        </p>
    ...
    public function wp_playlist_shortcode_override($html, $attr, $instance)
    {
        ...
        if ((!empty($attr['player']) && $attr['player'] === 'default') || (!empty($attr['type']) && $attr['type'] !== 'audio') || (empty($attr['ids']))) {
