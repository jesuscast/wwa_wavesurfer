...
class WooKiteKite {
	...
	public function __construct( $plugin ) {
		...
		dbDelta(
			...
			'.//image_up_to_date TINYINT(1) DEFAULT 0,
            'kite_data LONGTEXT DEFAULT "",
            PRIMARY KEY  (id),
		...
		// Cron for orders statuses
		// register_activation_hook(__FILE__, 'wookite_cron_hourly');
		// if (!wp_next_scheduled('wookite_cron_hourly_event'))
		// wp_schedule_event(time(), 'hourly', 'wookite_cron_hourly');
	}

	public function shutdown() {
		if ( ! empty( $this->posts_to_delete ) ) {
			// Delete data of any posts scheduled for destruction
	...
	public function request( $endpoint, $args = array() ) {
		$url = 'https://api.kite.ly/v3.0/' . $endpoint;
		$me = (
	...
	protected function is_order_done( $order_data ) {
		...
		foreach ( (array) $order_data['jobs'] as $job ) {
			if ( $job['status'] !== 'Shipped' && $job['status'] !== 'Cancelled' ) {
				if ( ! $order_data['test_order']
					|| (                    $job['status'] !== 'Sent to Printer'
					&& $job['status'] !== 'Received by Printer')
				) {
					return false;
				}
			}
	...
	public function get_posts_parents_ids( $posts_ids, $type = null ) {
		...
		$res = $wpdb->get_col(
			sprintf(
				'SELECT DISTINCT parent_id FROM ' . WOOKITE_POSTMETA_TABLE_NAME . '
	...
	public function get_product_range_data( $product_id ) {
		...
		return wookite_maybe_unserialize( $res[0] );
	}
	...
	public function get_related_products_ids( $post_ids ) {
		...
		$post_ids = array_merge(
			$post_ids,
			$this->get_variants_products_ids( $post_ids )
		);
		$pids = array();
	...
	public function product_image_params($data, $side='image', $print_image=false) {
		...
		if (!isset($image_data['rotate']) && isset($image_data['rotate_degrees'])) {
			$image_data['rotate'] = $image_data['rotate_degrees'];
	...
	public function products_column( $column, $pid ) {
		if ( $column == 'wookite' and $this->get_product_data( $pid ) !== false ) {
			printf(
				'<span class="wookitext tips wookite-in-list" data-tip="Kite %s">K</span>',
				__( 'product', 'wookite' )
			);
		}
	}
	...
	public function add_admin_order_action( $actions, $order ) {
		...
		if ( $kite_info['has_kite'] ) {
			if ( $kite_info['order_data']['done'] && ! $kite_info['liveable_test_order'] ) {
				return $actions;
			}
			...
			// Make buttons
			if ( empty( $kite_info['kite_id'] ) || $kite_info['liveable_test_order'] ) {
				$actions['wookite_query'] = array(
				'url' => wp_nonce_url( admin_url( 'admin-ajax.php?action=wookite_send_order_to_kite&order_id=' . $order->id ), 'wookite_send_order_to_kite' ),
				'name' => (
				...
				'url'       => wp_nonce_url( admin_url( 'admin-ajax.php?action=wookite_get_order_status&order_id=' . $order->id ), 'wookite_get_order_status' ),
				'name' => esc_attr(
					sprintf(
						'Kite %s: %s%s<br />%s',
		...
		if ( $kite_info['has_kite'] ) {
			...
			if ( empty( $kite_info['status'] ) || $kite_info['status'] === 'Error' || $kite_info['liveable_test_order'] ) {
				$v2 = sprintf(
					...
					wp_nonce_url( admin_url( 'admin-ajax.php?action=wookite_get_order_status&order_id=' . $order->id . '&output=new' ), 'wookite_get_order_status' ),
				...
				printf(
					...
					wp_nonce_url( admin_url( 'admin-ajax.php?action=wookite_send_order_to_kite&order_id=' . $order->id . '&output=new' ), 'wookite_send_order_to_kite' ),
	...
	public function add_order_item_data( $product, $item, $item_id ) {
		if ( $this->kite_info['has_kite'] ) {
			if ( $this->kite_info['only_kite'] || $this->get_product_data( $product->id ) !== false ) {
				...
				printf(
					'<td><span class="wookitext tips wookite-big" data-tip="%s<br />%s: %s">K</span></td>',
	...
	public function profit_table( $order, $kite_info = null, $kite_items = null ) {
		...
		if ( ! isset( $kite_items ) ) {
			...
			foreach ( $items as $id => $item ) {
				...
				$pids[] = (int) $item['variation_id'];
			}
			$pvs = $this->get_posts_data( $pids, true );
			foreach ( $items as $id => $item ) {
				if ( $pvs[ (int) $item['product_id'] ] && $pvs[ (int) $item['variation_id'] ] ) {
					$kite_items[] = $item;
				}
			}
		}
		...
		$res .= sprintf(
			...
			wc_price( $items_cost, array( 'currency' => $order->get_order_currency() ) )
		);
		...
		$res .= '<tr><td></td></tr>';
		if ( $od['total_to_reconcile']['currency'] == $order->get_order_currency() ) {
			$res .= sprintf(
	...
	public function add_order_details( $order_id ) {
		if ( $this->kite_info['has_kite'] ) {
			...
			if ( $processed ) {
				...
				echo $this->profit_table( $this->order, $ki, $this->kite_items );
				echo '</td><td></td><td>';
	...
	public function send_order_to_kite( $order ) {
		...
		$kite_items = array();
		foreach ( $items as $id => $item ) {
			$product_meta = $this->get_product_data( $item['product_id'] );
			$variation_meta = $this->get_variant_data( $item['variation_id'] );
	...
	protected function get_order_status_from_kite_by_id( $oid, $kite_id ) {
		...
		return $this->response(
			0, '', array(
			'kite_data' => apply_filters(
				'wookite_fetched_kite_data', $resp['json'], $oid, $kite_id
			),
			)
		);
	}
